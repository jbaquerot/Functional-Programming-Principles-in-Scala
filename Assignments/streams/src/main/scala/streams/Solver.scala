...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    if (initial.isEmpty) Stream.empty
      ...
      val more = for {
        path <- initial
        n <- newNeighborsOnly(neighborsWithHistory(path._1, path._2), explored)
      } yield n
      initial#::: from(more, explored ++ (more map (x=> x._1)).toSet)
      //val neighbors = newNeighborsOnly(initial, explored)
      //initial #::: from(neighbors, explored ++ (neighbors map (x=> x._1)).toSet)
    }
