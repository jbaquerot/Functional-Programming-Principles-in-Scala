...
abstract class TweetSet {
   ...
   * and be implemented in the subclasses? Subclasess
   */
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    ...
    if (p(this.elem) == true) 
      new NonEmpty(this.elem, this.left.filterAcc(p, acc), this.right.filterAcc(p, new Empty))
    else 
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = 
   TweetReader.allTweets.filter(t => google.exists(p => t.text.contains(p)))
   //TweetReader.allTweets
  lazy val appleTweets: TweetSet = 
    TweetReader.allTweets.filter(t => apple.exists(p => t.text.contains(p)))
